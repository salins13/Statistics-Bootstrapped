...
# Streamlit UI
st.title("Normal Distribution Analysis with Bootstrapping")

# Step 2: Take user input for sample sizes and number of samples
sample_sizes = st.text_input("Enter up to 4 sample sizes separated by commas (e.g., 500,50,10):")
...
if st.button("Analyze"):
    ...
    st.subheader("Smoothed Density Plots for Sample Distributions with Population Density Curve")
    ...
    st.subheader("Visualization of Sample Means and Confidence Intervals")
    fig, axes = plt.subplots(1, len(sample_sizes), figsize=(18, 6), sharey=True)
